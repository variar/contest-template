...
#include <type_traits>
	...
	#define JSON_DTO_CHECK_FOR_STD_OPTIONAL
		...
		#elif __has_include(<absl/types/optional.h>)
			#include <absl/types/optional.h>
			#define JSON_DTO_HAS_ABSL_OPTIONAL
		#endif
		#if defined(JSON_DTO_HAS_STD_OPTIONAL) || \
				defined(JSON_DTO_HAS_EXPERIMENTAL_OPTIONAL) || \
				defined(JSON_DTO_HAS_ABSL_OPTIONAL)
			#define JSON_DTO_SUPPORTS_STD_OPTIONAL
		#endif
	#endif

	#define JSON_DTO_CHECK_FOR_STD_VARIANT
	#if defined( _MSC_VER ) && !_HAS_CXX17
		// Visual C++ 14.* allows to include <variant> only in c++17 mode.
		#undef JSON_DTO_CHECK_FOR_STD_VARIANT
	#endif

	#if defined( JSON_DTO_CHECK_FOR_STD_VARIANT )
		#if __has_include(<variant>)
			#include <variant>
			#define JSON_DTO_HAS_STD_VARIANT
		#elif __has_include(<experimental/variant>)
			#include <experimental/variant>
			#define JSON_DTO_HAS_EXPERIMENTAL_VARIANT
		#elif __has_include(<absl/types/variant.h>)
			#include <absl/types/variant.h>
			#define JSON_DTO_HAS_ABSL_VARIANT
		#endif
		#if defined(JSON_DTO_HAS_STD_VARIANT) || \
				defined(JSON_DTO_HAS_EXPERIMENTAL_VARIANT) || \
				defined(JSON_DTO_HAS_ABSL_VARIANT)
			#define JSON_DTO_SUPPORTS_STD_VARIANT
		#endif
	#endif
#endif
...
#if defined(JSON_DTO_SUPPORTS_STD_VARIANT)
	#if defined(JSON_DTO_HAS_STD_VARIANT)
...
inline void
read_json_value(
	cpp17::variant<Types...> & v,
	const rapidjson::Value & object );

template< typename... Types >
inline void
write_json_value(
	const cpp17::variant<Types...> & v,
	rapidjson::Value & object,
	rapidjson::MemoryPoolAllocator<> & allocator );
#endif
...
template <typename U>
struct is_nullable_t<nullable_t<U>> : std::true_type
{
};
...
template <typename T>
struct is_custom_dto : std::false_type
{};
...
typename std::enable_if<!is_custom_dto<Dto>::value, void>::type
...
namespace details
{
	...
	struct for_each_type
	{
		...
		constexpr auto operator()(UnaryFunction f) const -> UnaryFunction
		{
	...
	const rapidjson::Value & object )
{
    ...
    details::for_each_type<Types...>{}([&v, &object, &hasValue, &typeIndex, &exceptions](auto value_from_stream)
    {
		typeIndex++;
        if (hasValue)
        {
        ...
        try 
        {
            ...
            std::stringstream ss;
            ss << "error reading value of type at index " << typeIndex << ": " << e.what();
...
write_json_value(
	...
	rapidjson::MemoryPoolAllocator<> & allocator )
{
	cpp17::visit([&](const auto& val) 
	{
		write_json_value( val, object, allocator );
	}, v);
}
#endif
